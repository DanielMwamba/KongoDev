...
const Hero = () => {
  ...
  return (
    <section className=" mt-16 relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary/5 to-secondary/10">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto w-full">
        ...
        <p className="mt-6 max-w-2xl text-center text-xl sm:text-2xl md:text-3xl text-muted-foreground animate-fade-in-up animation-delay-300">
          ...
          Découvrez, apprenez et partagez vos connaissances pour façonner
          l'avenir du web.
        </p>
        ...
        <div className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-16">
          <FeatureItem
            icon={Code}
            title="Codez"
            description="Perfectionnez vos compétences avec des défis de codage stimulants"
          />
          <FeatureItem
            icon={BookOpen}
            title="Apprenez"
            description="Explorez des tutoriels et des ressources de pointe"
            delay={300}
          />
          <FeatureItem
            icon={Share2}
            title="Partagez"
            description="Contribuez à la communauté et gagnez en visibilité"
            delay={600}
          />
        </div>
      </div>
...
const FeatureItem = ({ icon: Icon, title, description, delay = 0 }) => (
  <div
    className={`flex flex-col items-center animate-float animation-delay-${delay}`}
  >
